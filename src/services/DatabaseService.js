...
export const createUser = async user => {
  try {
    ...
    const emailKey = user.email ? user.email.replace(/[@.]/g, '_') : '';
    if (!emailKey) {
      throw new Error('Failed to generate valid email key');
    }
    ...
    try {
      await firestore().runTransaction(async transaction => {
        ...
        if (docSnapshot.exists) {
          ...
          result = {id: emailKey, ...existingData, ...userData};
        } else {
          // New document
          transaction.set(userRef, userData, {merge: true});
...
export const getTeacherEmailFromCode = async teacherCode => {
  try {
    ...
    try {
      ...
      throw new Error(`Failed to query teacher: ${queryError.message}`);
    }
    ...
    if (!teacherData) {
      console.warn('Teacher document exists but data is empty');
      return null;
    }
...
export const updateUser = async user => {
  try {
    // Validate input
    if (!user || typeof user !== 'object') {
      throw new Error('Invalid user object: must be a valid object');
    }
    ...
    try {
      ...
      console.error('Error writing user data:', writeError.message);
      throw new Error(`Failed to update/create user: ${writeError.message}`);
    }
      ...
      await firestore().runTransaction(async transaction => {
        ...
        if (!studentDoc.exists) {
          ...
          if (classIndex >= 0) {
            ...
            const datesPresent = Array.isArray(classItem.datesPresent)
              ? classItem.datesPresent
              : [];

            for (let i = 0; i < datesPresent.length; i++) {
              if (datesPresent[i] === currentDate) {
                throw new Error(
        ...
        if (!teacherDoc.exists) {
          ...
          let classIndex = -1;
          for (let i = 0; i < classes.length; i++) {
            if (classes[i] && classes[i].date === currentDate) {
              classIndex = i;
              break;
            }
          }
          ...
          if (classIndex >= 0) {
            ...
            const classItem = classes[classIndex];
            const present = Array.isArray(classItem.present)
              ? classItem.present
              : [];
            ...
            updatedClasses[classIndex] = {
              ...
              date: currentDate,
              present: [
                ...(Array.isArray(updatedClasses[classIndex].present)
...
export const teacherCancelsAttendance = async (
  ...
  try {
    ...
    if (!studentKey) {
      throw new Error('Failed to generate valid student key');
    }
    ...
    try {
      await firestore().runTransaction(async transaction => {
        ...
        if (!existingClass) {
          throw new Error('Class record is invalid');
        }

        const datesPresent = Array.isArray(existingClass.datesPresent)
        ...
        for (let i = 0; i < datesPresent.length; i++) {
          if (datesPresent[i] !== currentDate) {
            updatedDates.push(datesPresent[i]);
          }
        }
        ...
        for (let i = 0; i < teacherClasses.length; i++) {
          if (teacherClasses[i] && teacherClasses[i].date === currentDate) {
            teacherClassIndex = i;
            break;
          }
        }
        ...
        if (!teacherClass) {
          throw new Error('Teacher class record is invalid');
        }

        const presentStudents = Array.isArray(teacherClass.present)
      ...
      try {
        await firestore().runTransaction(async transaction => {
          ...
          if (!classesDoc.exists) {
            ...
            const dates = Array.isArray(classesData.dates)
              ? classesData.dates
              : [];
...
export const getAttendanceTeacherCurrentDate = async (
  ...
  try {
    ...
    try {
      ...
      console.error('Error fetching attendance record:', fetchError.message);
    ...
    for (let i = 0; i < data.classes.length; i++) {
      ...
      if (cls && cls.date && cls.date === currentDate) {
        todaysClass = cls;
        break;
      }
    }
    ...
    return Array.isArray(todaysClass.present) ? todaysClass.present : [];
  } catch (error) {
    ...
    while (attempts < maxAttempts) {
      try {
        await firestore().runTransaction(async transaction => {
          ...
          let students = Array.isArray(classData.students)
            ? classData.students
            : [];
...
export const getStudentAttendanceReport = async studentEmail => {
  try {
    ...
    let docSnapshot;
    try {
      docSnapshot = await attendanceRef.get();
    } catch (fetchError) {
      console.error('Error fetching student attendance:', fetchError.message);
      throw new Error(
    ...
    if (!docSnapshot.exists) {
      console.log(`No attendance found for student: ${emailKey}`);
      return null; // Consistent with original behavior
    }
    ...
    // Optional: Verify the email matches the input to detect collisions
    if (userData.studentEmail && userData.studentEmail !== studentEmail) {
      console.warn(
...
export const getTeachertAttendanceReport = async (classCode, teacherCode) => {
  try {
    // Validate input
    if (!classCode || typeof classCode !== 'string') {
      throw new Error('Class Code is required and must be a string');
    }
    if (!teacherCode || typeof teacherCode !== 'string') {
      throw new Error('Teacher Code is required and must be a string');
    }
    ...
    try {
      ...
      console.error('Error fetching teacher attendance:', fetchError.message);
      throw new Error(
    ...
    if (!docSnapshot.exists) {
      console.log(`No attendance record found for ${docKey}`);
      return null; // Consistent with original behavior
    }
